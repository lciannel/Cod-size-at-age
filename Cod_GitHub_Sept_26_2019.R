...
catchdata$cpue<-catchdata$NUMBER_FISH/(catchdata$DISTANCE_FISHED*catchdata$NET_WIDTH)
...
#Find out and add age of individuals in length file by first building an lt-age key.
...
#Fig. 1: Cod catches on cold and warm years over bottom temperatures and time series of cold pool index
...
dev.copy(tiff,'Fig1.tiff',width=7,height=7,units='in',res=600)
dev.off()



# MODEL 1, Table 1 (reference):
subdata<-na.exclude(fishdata[,c('lat.avg','lon.avg','STATIONID','LENGTH','SEX','LAT','LON','TEMP','AGE','COHORT')])
...
dev.copy(tiff,'Fig2.tif',width=10,height=5,units='in',res=600)
dev.off()


#Fig. S7: Map residuals for different age groups separately
...
dev.copy(jpeg,'FigS7.jpg',width=8,height=8,units='in',res=600)
dev.off()
...
dev.copy(jpeg,'FigS2.jpg',width=8,height=4,units='in',res=600)
dev.off()
...
map("worldHires",fill=T,col="grey",add=T)
dev.copy(tiff,'Fig3.tif',width=10,height=5,units='in',res=600)
dev.off()
...
for(i in 1:length(ages)){
	...
	#loess.age<-loess(log(cpue)~START_LONGITUDE*START_LATITUDE,data=age_data[age_data$AGEPLUS==ages[i],],span=0.1,degree=2)
#summary(lm(loess.age$fitted~log(na.exclude(age_data$cpue[age_data$AGE==ages[i]]))))
...
dev.copy(jpeg,'FigS1.jpg',width=8,height=7,units='in',res=600)
dev.off()
...
dev.copy(jpeg,'FigS3.jpg',width=12,height=8,units='in',res=600)
dev.off()
...
st_full<-read.table('/Users/lciannel/Documents/MyDocuments/Funding/NPRB/2015/Cod size at age/Analyses/GitHub/Station_Data_1992_2012.csv',sep=',',header=T)
...
lm.pos<-lm(lat1~lon1)
summary(lm.pos)
#Residual standard error: 0.2073 on 17 degrees of freedom
Multiple R-squared:  0.4726,	Adjusted R-squared:  0.4416 
#F-statistic: 15.24 on 1 and 17 DF,  p-value: 0.001143
...
for(i in 1:length(lon1)){
	line.index<-order(distance.function(coef(lm.pos)[1]+coef(lm.pos)[2]*lon1[i],lon1[i],latd,lond))[1]
	cohort.dist[i]<-distance.function(latd[line.index],lond[line.index],max(latd),min(lond))/1000
}
...
#Correlation of cohort effect with YEAR effect from Model 1 (Figure S4)
tmp<-na.exclude(subdata[,c('YEAR','LENGTH','COHORT','SEX','LON','LAT','TEMP','AGE')])
...
#GCV = 2526.3  Scale est. = 2520.9    n = 16236
AIC(gam8)
#173279.1
...
#R-sq.(adj) =  0.887   Deviance explained = 88.7%
#GCV = 2577.1  Scale est. = 2571.4    n = 16236
AIC(gam10.1)
#173602.1


####Threshold models
#Model 5: Additive effects of T.AGE1, and with threshold effect of cohort.dist (Best resonable model)
...
map("worldHires",fill=T,col="grey",add=T)
points(name$LON.AGE1[name$cohort.dist<th.best&name$AGE==1],name$LAT.AGE1[name$cohort.dist<th.best&name$AGE==1],pch=16, col='blue')
points(name$LON.AGE1[name$cohort.dist>=th.best&name$AGE==1],name$LAT.AGE1[name$cohort.dist>=th.best&name$AGE==1],pch=16, col='red')
...
dev.copy(jpeg,'FigS5.jpg',width=6,height=10,units='in',res=600)
dev.off()

#Fit threshold GAM model with best distance
subdata$cohort.loc<-ifelse(subdata$cohort.dist>=th.best,'2.East','1.West')
gam23.7<-gam(LENGTH~as.factor(cohort.loc)+factor(SEX)+s(LON,LAT)+s(TEMP,k=3)+s(AGE,k=3)+s(T.AGE1,k=3)-1,data=subdata)
summary(gam23.7)
quartz()
plot(gam23.7,all.terms=T,scale=0,pages=1)
#R-sq.(adj) =   0.89   Deviance explained = 98.8%
#GCV = 2494.1  Scale est. = 2488.5    n = 16236
AIC(gam23.7)
#173070.5

#Plot position and T.AGE1 effects estimated from gam23.7 (Fig 6 in paper)
dev.new(width=10,height=5)
par(mfrow=c(1,2),bg='white')
pred.subdata<-subdata[1:2,c('LENGTH','cohort.loc','SEX','LON','LAT','TEMP','AGE','T.AGE1')]
pred.subdata$SEX=rep(2,2);pred.subdata$LON=rep(-165,2);pred.subdata$LAT=rep(57,2);pred.subdata$AGE=rep(3,2);pred.subdata$TEMP=rep(2,2);pred.subdata$T.AGE1=rep(3,length=2);pred.subdata$cohort.loc=c('1.West','2.East');head(pred.subdata)
pred.fig<-predict(gam23.7,newdata=pred.subdata,se=T)
pred.fig$fit
plot(pred.fig$fit,xlim=c(0.5,2.5),pch=16,ylim=c(380,450),ylab='Average length (mm)',cex.axis=1.6,cex.lab=1.6,axes=F,xlab='')
axis(1,at=c(1,2),labels=c('West','East'),cex.axis=1.6);axis(2,cex.axis=1.6);box()
segments(c(0.7,1.7),c(pred.fig$fit[1],pred.fig$fit[2]),c(1.3,2.3),c(pred.fig$fit[1],pred.fig$fit[2]))
segments(c(0.7,1.7),c(pred.fig$fit[1]+1.96*pred.fig$se[1],pred.fig$fit[2]+1.96*pred.fig$se[1]),c(1.3,2.3),c(pred.fig$fit[1]+1.96*pred.fig$se[1],pred.fig$fit[2]+1.96*pred.fig$se[1]),lty=2)
segments(c(0.7,1.7),c(pred.fig$fit[1]-1.96*pred.fig$se[1],pred.fig$fit[2]-1.96*pred.fig$se[1]),c(1.3,2.3),c(pred.fig$fit[1]-1.96*pred.fig$se[1],pred.fig$fit[2]-1.96*pred.fig$se[1]),lty=2)
pred.subdata<-subdata[1:100,c('LENGTH','cohort.loc','SEX','LON','LAT','TEMP','AGE','T.AGE1')]
pred.subdata$SEX=rep(2,100);pred.subdata$LON=rep(-165,100);pred.subdata$LAT=rep(57,100);pred.subdata$AGE=rep(3,100);pred.subdata$TEMP=rep(2,100);pred.subdata$T.AGE1=seq(-0.5,4.5,length=100);pred.subdata$cohort.loc=rep('1.West',100);head(pred.subdata)
pred.fig<-predict(gam23.7,newdata=pred.subdata,se=T)
pred.fig$fit[1:10]
plot(pred.subdata$T.AGE1,pred.fig$fit,type='l',ylim=range(c(pred.fig$fit+2.2*pred.fig$se,pred.fig$fit-2.2*pred.fig$se)),ylab='Average length (mm)',xlab='Temperature (C)', cex.axis=1.6,cex.lab=1.6,lwd=2,)
lines(pred.subdata$T.AGE1,pred.fig$fit+1.96*pred.fig$se,lty=2)
lines(pred.subdata$T.AGE1,pred.fig$fit-1.96*pred.fig$se,lty=2)
dev.copy(tiff,'Fig6.tif',width=10,height=5,units='in',res=600)
dev.off()

#Model 6: Additive effects of cohort.dist, and with threshold effect of T.AGE1 
t.age1<-tapply(subdata$T.AGE1,subdata$COHORT,mean)
th.values<-seq(quantile(t.age1,0.2),quantile(t.age1,0.8),length=20)
aic.t1<-th.values*NA
for(i in 1:length(th.values)){
	gam.test<-gam(LENGTH~factor(SEX)+s(LON,LAT)+s(TEMP,k=3)+s(AGE,k=3)+s(cohort.dist,k=3)+as.factor(T.AGE1<th.values[i]),data=subdata)
	aic.t1[i]<-AIC(gam.test)
}
#AIC profile
th.best<-th.values[order(aic.dist1)[1]]
plot(th.values,aic.t1,type='b',xlab='Temp (C)',ylab='AIC')
abline(v=th.best,lty=2)

subdata$cohort.t<-ifelse(subdata$T.AGE1>=th.best,'Warm','Cold')
gam23.8<-gam(LENGTH~as.factor(cohort.t)+factor(SEX)+s(LON,LAT)+s(TEMP,k=3)+s(AGE,k=3)+s(cohort.dist,k=3)-1,data=subdata)
summary(gam23.8)
quartz()
plot(gam23.8,all.terms=T,scale=0,pages=1)
#R-sq.(adj) =  0.889   Deviance explained = 98.8%
#GCV = 2521.8  Scale est. = 2516.1    n = 16236
AIC(gam23.8)
#173249.6

#Calculate distance between various cohorts
dist.matrix<-NA*matrix(1:(length(lon1)*length(lat1)),nrow=length(lon1),ncol=length(lat1),dimnames=list(names(lon1),names(lat1)))
for(i in 1:length(lon1)){
	dist.matrix[i,]<-round(distance.function(lat1[i],lon1[i],lat1,lon1)/1000,0)}
order(dist.matrix,decreasing=T)[1]


#VCGAM: Temperature effect for each age group (set with 'target.age')
ages<-unique(age_data$AGEPLUS)
Rsq.all<-(1:length(ages))*NA
all.slopes<-NULL
all.model<-as.list(1:length(ages))

for(i in 1:length(ages)){
target.age<-ages[i]
subdata<-na.exclude(age_data[age_data$AGEPLUS==target.age&age_data$cpue>0,])
subdata$tst<-(subdata$GEAR_TEMPERATURE-mean(subdata$GEAR_TEMPERATURE))/sd(subdata$GEAR_TEMPERATURE)
subdata$t.star<-subdata$tst+1.1*abs(min(subdata$tst))
age.t<-gam(log(cpue+1)~factor(YEAR)+s(lon.avg,lat.avg)+s(lon.avg,lat.avg, by=t.star),data=subdata)
Rsq.all[i]<-summary(age.t)$r.sq

#Get SST coefficients and calculate significance
covariate<-subdata$t.star
pred<-predict(age.t,type='terms')[,3]
subdata$pred.se<-predict(age.t,type='terms',se.fit=T)[[2]][,3]/covariate
subdata$slope<-pred/covariate
agg.sub<-aggregate(slope~lon.avg+lat.avg,data=subdata,FUN=mean)
agg.sub$pred.se<-aggregate(pred.se~lon.avg+lat.avg,data=subdata,FUN=mean)$pred.se
agg.sub$AGEPLUS<-ages[i]
all.slopes<-rbind(all.slopes,agg.sub)
all.model[[i]]<-age.t}

#Plotting results: Fig. 4
jet.colors<-colorRampPalette(gray(seq(1,0.2,length=10)))#Defined as jet in myvis.gam
max.slope<-max(abs(all.slopes$slope))
tiff('Fig4.tif',width=10,height=7,res=600,units='in')
par(mfrow=c(3,3),mai=c(0.3,0.4,0.3,0.4))
ages<-unique(age_data$AGEPLUS)
for(i in 1:length(ages)){
pos.slope.se<-all.slopes$slope[all.slopes$slope>0&all.slopes$AGEPLUS==ages[i]]-1.96* all.slopes$pred.se[all.slopes$slope>0&all.slopes$AGEPLUS==ages[i]]
sign.pos<-pos.slope.se>0
neg.slope.se<-all.slopes$slope[all.slopes$slope>0&all.slopes$AGEPLUS==ages[i]]+1.96* all.slopes$pred.se[all.slopes$slope>0&all.slopes$AGEPLUS==ages[i]]
sign.neg<-neg.slope.se<0
age.t<-all.model[[i]]
myvis.gam(age.t,view=c('lon.avg','lat.avg'),plot.type="contour",color="jet",too.far=.03,ylab="Lat",xlab="Lon",main=paste('Age',ages[i]))
gradientLegend(valRange=range(age.t$fitted), color = gray(seq(1,0.2,length=10)), pos = c(-178.2,54.8,-176.7,56.8),coords = TRUE,n.seg = 4, border.col = "black",dec=2)
map('world',fill=T,col='grey',add=T)

symbols(all.slopes$lon.avg[all.slopes$slope>0&all.slopes$AGEPLUS==ages[i]][sign.pos], all.slopes$lat.avg[all.slopes$slope>0&all.slopes$AGEPLUS==ages[i]][sign.pos],circles= all.slopes$slope[all.slopes$slope>0&all.slopes$AGEPLUS==ages[i]][sign.pos],inches=0.08*max(all.slopes$slope[all.slopes$slope>0&all.slopes$AGEPLUS==ages[i]])/max.slope,bg=alpha('red',f=0.5),fg=alpha('grey',f=0.7),add=T)
try(symbols(all.slopes$lon.avg[all.slopes$slope<0&all.slopes$AGEPLUS==ages[i]][sign.neg], all.slopes$lat.avg[all.slopes$slope<0&all.slopes$AGEPLUS==ages[i]][sign.neg],circles=abs(all.slopes$slope[all.slopes$slope<0&all.slopes$AGEPLUS==ages[i]][sign.neg]),inches=0.08*abs(min(all.slopes$slope[all.slopes<0&all.slopes$AGEPLUS==ages[i]]))/max.slope,bg=alpha('blue',f=0.5),fg=alpha('grey',f=0.7),add=T),TRUE)
legend.bubble<-round(c(seq(max.slope,0,length=4)),1)
symbols(rep(-160.5,3),c(60.8,60.4,60),circle=legend.bubble[1:3],add=T,inches=0.08*max(legend.bubble)/max.slope,bg='grey')
text(rep(-158.9,3),c(60.8,60.4,60),labels=as.character(legend.bubble)[1:3],cex=1)
text(-160,61.5,'ln(n/ha)')
text(-176.5,57.3,'ln(n/ha)')
}
dev.off()
